  function initialize() {
    ...
    var map = new google.maps.Map(mapDiv, {
      center: new google.maps.LatLng(-23.621727784423868, -46.68132134587904),
      zoom: 18,
    ...
    layer = new google.maps.FusionTablesLayer({
      ...
      query: {
        select: "col15",
        from: "1AGKEUkdMpr_bgsZtrf4YLOcMzfnVZ9CcZYJpwgnh",
        where: ""
      },
      options: {
        styleId: 2,
    ...
    var tipoLampada = e.row['TipoLampada'].value;
    $('#TipoLampada option:contains("'+e.row['TipoLampada'].value+'")').prop('selected', true);
    $('#Brao option[value="'+e.row['Braco'].value+'"]').prop('selected', true);
    $('#Luminaria option:contains("'+e.row['Luminaria'].value+'")').prop('selected', true);
    $('#PotnciadaLmpada').prop('value', e.row['Lampada'].value);
    $('#Reator').prop('value', e.row['Reator'].value);
    $('#ReleFotocelula').prop('checked', (e.row['Rele'].value == "1"));
    $('#CoordenadaX').prop('value', e.row['X'].value);
    $('#CoordenadaY').prop('value', e.row['Y'].value);
    var dataR = e.row['DataRecenceamento'].value.split(/\//);
    $("#DataRecenceamento").prop('value', [dataR[2],dataR[1],dataR[0]].join('-'));
    var dataR = e.row['DataValidacao'].value.split(/\//);
    $("#DataValidao").prop('value', [dataR[2],dataR[1],dataR[0]].join('-'));
    $('#Vidatilanos').prop('value', e.row['VidaUtil'].value/365.0);
    $('#TipodeVia option[value="'+e.row['TipoVia'].value+'"]').prop('selected', true);
    $('#ClassificaoTrfego option[value="'+e.row['Trafego'].value+'"]').prop('selected', true);
    $('#ClassificaoVeculo option[value="'+e.row['Veiculo'].value+'"]').prop('selected', true);
    var geocoder = new google.maps.Geocoder();
    var latlon = new google.maps.LatLng(e.row['Y'].value,e.row['X'].value);
    geocoder.geocode( {'location':latlon}, function(results, status) {
        var Logradouro = results[0].address_components[1].short_name+', '+results[0].address_components[0].short_name;
        $("#Logradouro").prop('value', Logradouro);
    });
});
...
{
if (this == "SP"){
UF.append($("<option selected/>").val(this).text(this));
}
else
{
UF.append($("<option />").val(this).text(this));
}
});

var Mun = $("#Cidade")
var selectedUF = UF[0].selectedIndex;
console.log(selectedUF);
Mun.find('option').remove()
$.each(munic[selectedUF], function()
{
Mun.append($("<option />").val(this).text(this));
});
$("#Cidade option[value='SAO PAULO']").prop('selected', true);
